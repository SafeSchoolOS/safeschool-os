 ...
 * Uses HMAC-authenticated HTTP (same signing scheme as SyncClient) over plain
 * HTTP for LAN communication.
 *
 * - Push: forward connector events matching route entityTypes to peer VMs
 * - Pull: fetch analytics results back from peers (e.g., SafeSchool → SafeSchool)
 * - Subscription-gated: only federates if target product is in activation key
 */

import crypto from 'node:crypto';
import { createLogger, type FederationConfig, type FederationPeer, type FederationRoute, type ProductFlag } from '@edgeruntime/core';
...
export class FederationManager {
  ...
  /**
   * Register a handler for inbound federated events. Handlers can return analytics
   * entities that get queued for push back to the originating product.
   */
  ...
  onConnectorEvents(_connectorName: string, events: Record<string, unknown>[]): void {
    // Skip events that were federated TO us (prevent loops)
   ...
   * Returns non-analytics events for ConnectorEventBuffer insertion.
   */
  handleInboundEvents(fromProduct: string, events: Record<string, unknown>[]): Record<string, unknown>[] {
    ...
    // If these are analytics, store in analytics buffer
    ...
    // Invoke registered handlers (e.g., SafeSchool FederationProcessor)
    for (const handler of this.eventHandlers) {
      try {
        const results = handler(fromProduct, tagged);
        if (results.length > 0) {
          // Queue handler-produced analytics for push to peers
          this.pendingEvents.push(...results);
          // Also store in local analytics buffer
  ...
  private async pushTick(): Promise<void> {
    ...
    for (const [product, state] of this.peerStates) {
      ...
      // Collect all entity types for this peer
      const allowedTypes = new Set(routes.flatMap(r => r.entityTypes));

      // Filter events by type (check both 'type' and 'eventType' fields)
