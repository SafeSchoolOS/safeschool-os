 ...
 * Delegates camera operations to @safeschoolos/adapters MilestoneAdapter
 * while maintaining the EdgeRuntime connector lifecycle and GSOC
 * event normalization.
 */

import { BaseConnector, type ConnectorConfig } from '@edgeruntime/connector-framework';
import { MilestoneAdapter } from '@safeschoolos/adapters/cameras';
...
export interface MilestoneConfig extends ConnectorConfig {
  serverUrl: string;           // e.g. https://milestone-server:443
  username: string;
  password: string;
  verifySsl: boolean;
...
export class MilestoneXProtectConnector extends BaseConnector {
  ...
  constructor(name: string, config: MilestoneConfig) {
    ...
    // Initialize the shared camera adapter for camera discovery/streaming
  ...
  async connect(): Promise<boolean> {
    try {
      // Authenticate via IDP connect token endpoint
  ...
  async fetchEvents(since?: Date): Promise<Record<string, unknown>[]> {
    ...
    try {
      ...
      // Fetch event lines (motion, analytics, tamper, etc.)
  ...
  // ─── Camera Operations (delegated to adapter) ─────────────────────
  ...
  /**
   * Create a bookmark on a camera (e.g., when an access event occurs nearby).
   */
