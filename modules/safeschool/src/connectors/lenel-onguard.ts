 ...
 *
 * Ported from AccessControlGSOC/app/connectors/lenel_connector.py.
 ...
 *
 * Delegates vendor API calls to @safeschoolos/adapters SicunetAdapter
...
import { SicunetAdapter } from '@safeschoolos/adapters/access-control';
...
export class LenelOnGuardConnector extends BaseConnector {
  ...
  constructor(name: string, config: LenelConfig) {
    ...
    // Initialize the shared adapter for lock/unlock/lockdown commands
    this.adapter = new SicunetAdapter();
  }

  async connect(): Promise<boolean> {
    try {
      // Connect the shared adapter with our credentials
  ...
  // ─── Door Commands (delegated to adapter) ─────────────────────────
